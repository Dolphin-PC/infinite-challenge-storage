import React from "react";

export default function AccountCircle() {
  return (
    <>
      <svg
        width="50"
        height="50"
        viewBox="0 0 50 50"
        fill="currentColor"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g clip-path="url(#clip0_4_10)">
          <path
            d="M25 4.16666C13.5 4.16666 4.16669 13.5 4.16669 25C4.16669 36.5 13.5 45.8333 25 45.8333C36.5 45.8333 45.8334 36.5 45.8334 25C45.8334 13.5 36.5 4.16666 25 4.16666ZM25 10.4167C28.4584 10.4167 31.25 13.2083 31.25 16.6667C31.25 20.125 28.4584 22.9167 25 22.9167C21.5417 22.9167 18.75 20.125 18.75 16.6667C18.75 13.2083 21.5417 10.4167 25 10.4167ZM25 40C19.7917 40 15.1875 37.3333 12.5 33.2917C12.5625 29.1458 20.8334 26.875 25 26.875C29.1459 26.875 37.4375 29.1458 37.5 33.2917C34.8125 37.3333 30.2084 40 25 40Z"
            fill="currentColor"
          />
        </g>
        <defs>
          <clipPath id="clip0_4_10">
            <rect width="50" height="50" fill="white" />
          </clipPath>
        </defs>
      </svg>
    </>
  );
}
